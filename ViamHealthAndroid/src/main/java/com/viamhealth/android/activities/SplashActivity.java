...
public class SplashActivity extends BaseActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private ArrayList<User> getFamilyFromCursor(Cursor cursor) {
        LogUtils.LOGD(TAG, "getFamilyFromCursor.. ");
        if(cursor!=null && cursor.moveToFirst()){
            LogUtils.LOGD(TAG, "getFamilyFromCursor.. inside if ");
            familyMembers.clear();
            while(!cursor.isAfterLast()){
                LogUtils.LOGD(TAG, "getFamilyFromCursor.. inside while ");
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if(cursor!=null){
            LogUtils.LOGD(TAG, "Cursor is not null " );
            switch (loader.getId()){
                case USERS_LOADER:
                    LogUtils.LOGD(TAG, " got correct loader id " );
                    getFamilyFromCursor(cursor);
                    LogUtils.LOGD(TAG, " got family" );
                    moveToHomeScreen();
                    break;
            ...
            LogUtils.LOGD(TAG, "Cursor is null " );
        }
    }

    private void moveToHomeScreen() {
        LogUtils.LOGD(TAG, " trying in Home Screen " );
