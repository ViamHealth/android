...
public class functionClass {
	...
	// function for call signup service
		public String SignUp(String username,String password) {
			...
			responseString = client.getResponse();
				...
				try {
				 jObject = new JSONObject("{\"rs\":["+responseString+"]}");
				 Log.e("TAG","res : " + "{\"rs\":"+responseString+"}");
				 jarray = jObject.getJSONArray("rs");
					 if(jarray.length()>0){
						 String msg =jarray.getJSONObject(0).getString("username");
						 if(msg.contains("@")){
							 responsetxt="0";
						 }
					 }
    ...
    public Profile getLoggedInUser() {
        ...
        appPrefs.setLoggedInUser(processUserResponse(responseString));
        return pd;
    }

	// function for get family member list
	public List<User> GetFamilyMember(User user) {
		...
		responseString = client.getResponse();
			...
			try {
				 Log.e("TAG","first name is " + "{\"rs\":" +responseString+"}");
			 jObject = new JSONObject("{\"rs\":" +responseString+"}");
			 jarray = jObject.getJSONArray("rs");
			   appPrefs.setMenuList("");
				 for (int i = 0; i < jarray.length(); i++) {
					 appPrefs.setMenuList(appPrefs.getMenuList().toString()+ jarray.getJSONObject(i).getString("first_name") + 
							 " " +jarray.getJSONObject(i).getString("last_name")+",");
					 JSONObject c = jarray.getJSONObject(i);

                     Profile pd = new Profile();
                     pd.setEmail(c.getString("email"));
                     pd.setFirstName(c.getString("first_name"));
                     pd.setLastName(c.getString("last_name"));
                     String gender = c.getJSONObject("profile").getString("gender");
                     //if(gender=="")
                     pd.setGender(Gender.Male);

                     pd.setFbProfileId(c.getString("fbprofileid"));

					 lstFamily.add(pd);
				 }
				 ga = new Global_Application();
				 Global_Application.lstfamilyglobal = lstFamily;
	...
	// function for get family member list
	public List<Profile> GetUserProfile(Long userId) {
		List<Profile> lstFamily = new ArrayList<FamilyData>();
			 ...
			 jarray = jObject.getJSONArray("rs");
			   appPrefs.setMenuList("");
				 for (int i = 0; i < jarray.length(); i++) {
                     ...
                     Profile pd = new Profile();
                     //pd.setEmail();
					 	appPrefs.setUserid(c.getString("id").toString());
					  	appPrefs.setUsername(c.getString("username").toString());
					  	appPrefs.setFirstname(c.getString("first_name").toString());    
					  	appPrefs.setLastname(c.getString("last_name").toString());
					    appPrefs.setProfilepic(c.getJSONObject("profile").getString("profile_picture_url").toString());
					    Log.e("TAG","url : " + appPrefs.getProfilepic());
					    appPrefs.setGender(c.getJSONObject("profile").getString("gender").toString());
						appPrefs.setBornon(c.getJSONObject("profile").getString("date_of_birth").toString());
						lstFamily.add(new FamilyData(c.getString("id").toString(), c.getString("username").toString(),
								c.getString("email").toString(), c.getString("first_name").toString(), 
								c.getString("last_name").toString(), c.getJSONObject("profile").getString("profile_picture_url").toString()));
				 }
    ...
    private User processUserResponse(String jsonResponse) {
    ...
    private Profile processProfileResponse(String jsonProfile){

        Profile pd = new Profile();

        try{
            /* need to deserialize Profile object */
            JSONObject jsonProfile = new JSONObject(jsonProfile);
            pd.setGender(Gender::formEnum(jsonProfile.getString("gender")));
            SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
            pd.setDob(formatter.parse(jsonProfile.getString("date_of_birth")));
            pd.setBloodGroup(BloodGroup::formEnum(jsonProfile.getInt("blood_group")));
            pd.setFbProfileId(jsonProfile.getLong("fb_profile_id"));
            pd.setOrganization(jsonProfile.getString("organization"));
            pd.setProfilePicURL(jsonProfile.getString("profile_picture_url"));

            Profile.Location location = new Profile.Location();
    ...
    public User updateUser(User user) {
        ...
        user = updateUser(user.getId(), user.getFirstName(), user.getLastName());

        //Update profile data
        user.setProfile(updateProfile(user.getId(), user.getProfile()));

        return user;
    }

    // function for call add profile service
    public String updateUserProfile(Profile data) {
        String	responsetxt="1";
        String baseurlString = Global_Application.url+"/users/"+data.getId()+"/profile/";
        Log.e("TAG","url is : " + baseurlString);

        RestClient client = new RestClient(baseurlString);
        client.AddParam("gender", data.getGender().key());
        SimpleDateFormat formater = new SimpleDateFormat("yyyy-MM-dd");
        client.AddParam("date_of_birth",formater.format(data.getDob()));
        client.AddParam("fb_profile_id", data.getFbProfileId());
        client.AddParam("organization", data.getOrganization());
        client.AddParam("street", data.getLocation().getStreet());
        client.AddParam("city", data.getLocation().getCity());
        client.AddParam("state", data.getLocation().getState());
        client.AddParam("country", data.getLocation().getCountry());
        client.AddParam("zip_code", data.getLocation().getZip());
        client.AddParam("lattitude", data.getLocation().getLattitude());
        client.AddParam("longitude", data.getLocation().getLongitude());
        client.AddParam("address", data.getLocation().getAddress());
        client.AddParam("phone_number", data.getMobileNumber());
        ...
        try {
            ...
            responsetxt=jObject.getString("add");
            Log.e("TAG","login is " + responsetxt);
            if(responsetxt.toString().equals("0")){
                appPrefs.setMenuList("");
                jarray = jObject.getJSONArray("names");
                for (int i = 0; i < jarray.length(); i++) {
                    appPrefs.setMenuList(appPrefs.getMenuList().toString()+ jarray.getString(i) + ",");
                }
                Log.e("TAG","Data is " + appPrefs.getMenuList().toString());
            }
        } catch (JSONException e) {
	...
	public String inviteUser(String username, String firstname,String lastname){
		String	responsetxt="1";
		String baseurlString = Global_Application.url+"users/";    
		Log.e("TAG","url is : " + baseurlString);
		...
		try {
			 ...
			 jarray = jObject.getJSONArray("rs");
				 if(jarray.length()>0){
					 String msg =jarray.getJSONObject(0);
					 if(msg.contains("@")){
						 responsetxt="0";
					 }else{
						 responsetxt=msg;
